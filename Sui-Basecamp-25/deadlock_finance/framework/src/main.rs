...
async fn handle_client(mut stream: TcpStream) -> Result<(), Box<dyn Error>> {
    ...
    let modules = vec!["challenge", "coin", "staking"];
    let sources = vec!["challenge", "coin", "staking"];
    ...
    //sui_ctf_framework::set_clock(&mut adapter, 1746012087000000000).await;

    match sui_ctf_framework::view_object(&mut adapter, FakeID::Known(SUI_CLOCK_OBJECT_ID)).await {
        Ok(output) => {
            println!("OUR Output: {:#?}", output);
            output.unwrap()
        }
        Err(_error) => {
            println!("[SERVER] Error viewing the object with ID CLOCK");
            "error when viewing the object".into()
        }
    };

    let mut args_create : Vec<SuiValue> = Vec::new();
    let treasury_cap = SuiValue::Object(FakeID::Enumerated(2, 2), None); 
    let clock = SuiValue::ImmShared(FakeID::Known(SUI_CLOCK_OBJECT_ID), None);
    args_create.push(treasury_cap);
    args_create.push(clock.clone());
    ...
    let challenge_obj = SuiValue::Object(FakeID::Enumerated(5, 0), None);
    ...
    let mut is_solved_args : Vec<SuiValue> = Vec::new();
    let challenge_obj = SuiValue::Object(FakeID::Enumerated(5, 0), None);
    is_solved_args.push(challenge_obj);
    is_solved_args.push(clock.clone());

    match sui_ctf_framework::call_function(
        &mut adapter,
        chall_addr,
        "challenge",
        "is_solved",
        ...
        Ok(output) => {
            let message = "Error: Invalid Solution!";
            println!("[SERVER] {}", message);
            stream.write(message.as_bytes()).unwrap();
        },
        Err(e) => {
            let check_string = "Arithmetic error, stack overflow, max value depth, etc";
            if e.to_string().contains(check_string) {
