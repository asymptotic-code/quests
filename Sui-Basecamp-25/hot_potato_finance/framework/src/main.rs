...
async fn handle_client(mut stream: TcpStream) -> Result<(), Box<dyn Error>> {
    ...
    let modules = vec!["challenge", "oracle", "reward_pool", "deposit_coin", "reward_coin"];
    let sources = vec!["challenge", "oracle", "reward_pool", "deposit_coin", "reward_coin"];
    ...
    let rewards_treasury_cap = SuiValue::Object(FakeID::Enumerated(2, 4), None); 
    let deposits_treasury_cap = SuiValue::Object(FakeID::Enumerated(2, 3), None); 
    args_create.push(rewards_treasury_cap);
    args_create.push(deposits_treasury_cap);
